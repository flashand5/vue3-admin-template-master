...
<el-card>
  <div class="scroll-y">
      ...
      <el-form ref="refsearchForm" :inline="true" class="demo-searchForm ml-3">
        ...
        <el-form-item label-width="0px" label="" prop="status" label-position="left">
          <el-select v-model="searchForm.status" clearable placeholder="搜索字段" class="widthPx-120">
            ...
            <el-option label="模板名称" value="prcInfoTitle" />
            <el-option label="模板描述" value="prcInfoDesc" />
            <el-option label="模板状态" value="prcInfoStatus" />
          </el-select>
        </el-form-item>
      </el-form>
      <!--查询按钮-->
    ...
    >
      ...
      <el-table-column align="center" prop="prcInfoId" label="模板编号" sortable="custom" min-width="70" />
      <el-table-column align="center" prop="prcInfoTitle" label="模板名称" sortable="custom" min-width="70" />
      <el-table-column align="center" prop="prcInfoDesc" label="模板描述" width="240" />
      <el-table-column align="center" prop="taskId" label="任务编号" sortable="custom" min-width="70" />
      <el-table-column align="center" prop="prcInfoStatus" label="模板状态" sortable="custom" min-width="70" >
        <template #default="scope">
        <span v-if="scope.row.prcInfoStatus===0">
              <el-tag type="success">可用</el-tag>
            </span>
          <span v-else-if="scope.row.prcInfoStatus===1">
              <el-tag type="danger">禁用</el-tag>
            </span>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="classifyName" label="模板分类" sortable="custom" min-width="70" />
      <!--点击操作-->
      <el-table-column align="center" label="操作" fixed="right" width="120px">
        <template #default="{ row }">
          <el-button type="text" size="small" @click="launchBtnClick(row)">发起</el-button>
    ...
    <!--    发起流程，弹窗子组件-->
    <LaunForm v-if="launchFrom" ref="refVciForm" @hideComp="closeLaunch" />
  </div>
</el-card>
</template>
...
// 导入编辑弹窗的子组件
import VciForm from './processForm.vue'
import LaunForm from './processLaunch.vue'
//获取子组件实例（定义一个操作属性的方法）
...
let selectPageReq = () => {
  let reqConfig = {
    url: '/system/processinfo/selectListProcessInfo',
    method: 'post',
    data: getSearchData(),
    isParams: false,
...
//发起点击事件
let launchBtnClick = (row) => {
  launchFrom.value = true
  nextTick(() => {
    refVciForm.value.showModal(row)
  })
}
...
let deleteByIdReq = (id) => {
  return axiosReq({
    url: '/system/processinfo/deleteProcess',
    data: { prcInfoId: id },
    isParams: false,
    method: 'post',
    bfLoading: true
...
let tableDelClick = async (row) => {
  await elConfirm('确定', `您确定要删除【${row.prcInfoTitle}】吗？`)
  deleteByIdReq(row.prcInfoId).then(() => {
    selectPageReq()
    elMessage(`【${row.prcInfoTitle}】删除成功`)
  })
}
//批量删除
const multiDelBtnClick = async () => {
  ...
  rowDeleteIdArr = multipleSelection.value.map((mItem) => {
    deleteNameTitle = deleteNameTitle + mItem.prcInfoTitle + ','
    return mItem.prcInfoId
  })
  ...
  axiosReq({
    url: `/system/processinfo/deleteBatchProcess`,
    data,
    method: 'post',
    bfLoading: true
  }).then(() => {
