...
<el-card>
  <div class="scroll-y">
      ...
      <el-form ref="refsearchForm" :inline="true" class="demo-searchForm ml-3">
        ...
        <el-form-item label-width="0px" label="" prop="status" label-position="left">
          <el-select v-model="searchForm.status" clearable placeholder="搜索字段" class="widthPx-120">
            <el-option label="任务编号" value="prctaskid" />
            <el-option label="任务名称" value="prctaskname" />
            <el-option label="任务信息" value="prctaskdesc" />
            <el-option label="任务描述" value="prctaskinfo" />
            <el-option label="任务指令" value="prctaskcomm" />
          </el-select>
        </el-form-item>
      </el-form>
      <!--查询按钮-->
    ...
    >
      ...
      <el-table-column align="center" prop="prcTaskId" label="任务编号" sortable="custom" min-width="70" />
      <el-table-column align="center" prop="prcTaskName" label="任务名称" sortable="custom" min-width="70" />
      <el-table-column align="center" prop="prcTaskDesc" label="任务信息" width="240" />
      <el-table-column align="center" prop="prcTaskComm" label="任务指令" sortable="custom" min-width="240" />
      <el-table-column align="center" prop="personName" label="创建者" sortable="custom" min-width="70" />
      <!--点击操作-->
...
// 导入编辑弹窗的子组件
import VciForm from './proceStackForm.vue'
//获取子组件实例（定义一个操作属性的方法）
...
let selectPageReq = () => {
  let reqConfig = {
    url: '/system/processtack/selectProcesstack',
    method: 'post',
    data: getSearchData(),
    isParams: false,
...
let deleteByIdReq = (id) => {
  return axiosReq({
    url: '/system/processtack/deleteProcesstack',
    data: { prcTaskId: id },
    isParams: false,
    method: 'post',
    bfLoading: true
...
let tableDelClick = async (row) => {
  await elConfirm('确定', `您确定要删除【${row.prcTaskName}】吗？`)
  deleteByIdReq(row.prcTaskId).then(() => {
    selectPageReq()
    elMessage(`【${row.prcTaskName}】删除成功`)
  })
}
//批量删除
const multiDelBtnClick = async () => {
  ...
  rowDeleteIdArr = multipleSelection.value.map((mItem) => {
    deleteNameTitle = deleteNameTitle + mItem.prcTaskName + ','
    return mItem.prcTaskId
  })
  ...
  axiosReq({
    url: `/system/processtack/deleteBatchProcesstack`,
    data,
    method: 'post',
    bfLoading: true
  }).then(() => {
