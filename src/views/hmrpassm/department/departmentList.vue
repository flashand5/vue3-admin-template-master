<template>
  <div class="scroll-y">
      ...
      <el-form ref="refsearchForm" :inline="true" class="demo-searchForm ml-3">
        ...
        <el-form-item label-width="0px" label="" prop="status" label-position="left">
          <el-select v-model="searchForm.status" clearable placeholder="搜索字段" class="widthPx-120">
            <el-option label="部门编号" value="departmentId" />
    ...
    >
      ...
      <el-table-column align="center" prop="departmentId" label="部门编号" sortable="custom" min-width="70" />
      <el-table-column align="center" prop="departmentName" label="部门名称" sortable="custom" min-width="70" />
      <el-table-column align="center" prop="departmentDesc" label="部门描述" min-width="240" />
      <el-table-column align="center" prop="personName" label="负责人" sortable="custom" width="70" />
      <!--点击操作-->
...
// 导入编辑弹窗的子组件
import VciForm from './departmentForm.vue'
//获取子组件实例（定义一个操作属性的方法）
...
let selectPageReq = () => {
  let reqConfig = {
    url: '/system/department/selectListDepartment',
    method: 'post',
    data: getSearchData(),
    isParams: false,
...
let tableDelClick = async (row) => {
  await elConfirm('确定', `您确定要删除【${row.departmentName}】吗？`)
  deleteByIdReq(row.departmentId).then(() => {
    selectPageReq()
    elMessage(`【${row.departmentName}】删除成功`)
  })
}
//批量删除
const multiDelBtnClick = async () => {
  ...
  rowDeleteIdArr = multipleSelection.value.map((mItem) => {
    deleteNameTitle = deleteNameTitle + mItem.departmentName + ','
    return mItem.departmentId
  })
  ...
  axiosReq({
    url: `/system/department/deleteBatchDepartment`,
    data,
    method: 'post',
    bfLoading: true
  }).then(() => {
