...
<el-card>
  <div class="scroll-y">
      ...
      <el-form ref="refsearchForm" :inline="true" class="demo-searchForm ml-3">
        ...
        <el-form-item label-width="0px" label="" prop="status" label-position="left">
          <el-select v-model="searchForm.status" clearable placeholder="搜索字段" class="widthPx-120">
            <el-option label="分类编号" value="classifyId" />
            <el-option label="分类名称" value="classifyName" />
            <el-option label="负责人姓名" value="personName" />
          </el-select>
        </el-form-item>
      </el-form>
      <!--查询按钮-->
    ...
    >
      ...
      <el-table-column align="center" prop="classifyId" label="分类编号" sortable="custom" min-width="70" />
      <el-table-column align="center" prop="classifyName" label="分类名称" sortable="custom" min-width="70" />
      <el-table-column align="center" prop="classifyDesc" label="分类描述" width="240" />
      <el-table-column align="center" prop="personName" label="负责人姓名" sortable="custom" min-width="70" />
      <el-table-column align="center" prop="classifyTime" label="创建时间" sortable="custom" min-width="70" />
      <!--点击操作-->
...
// 导入编辑弹窗的子组件
import VciForm from './classifyForm.vue'
//获取子组件实例（定义一个操作属性的方法）
...
let selectPageReq = () => {
  let reqConfig = {
    url: '/system/classify/selectClassify',
    method: 'post',
    data: getSearchData(),
    isParams: false,
...
let deleteByIdReq = (id) => {
  return axiosReq({
    url: '/system/classify/deleteClassify',
    data: { classifyId: id },
    isParams: false,
    method: 'post',
    bfLoading: true
...
let tableDelClick = async (row) => {
  await elConfirm('确定', `您确定要删除【${row.classifyName}】吗？`)
  deleteByIdReq(row.classifyId).then(() => {
    selectPageReq()
    elMessage(`【${row.classifyName}】删除成功`)
  })
}
//批量删除
const multiDelBtnClick = async () => {
  ...
  rowDeleteIdArr = multipleSelection.value.map((mItem) => {
    deleteNameTitle = deleteNameTitle + mItem.classifyName + ','
    return mItem.classifyId
  })
  ...
  axiosReq({
    url: `/system/classify/deleteBatchClassify`,
    data,
    method: 'post',
    bfLoading: true
  }).then(() => {
