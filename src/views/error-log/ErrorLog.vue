...
let selectPageReq = () => {
  ...
  let reqConfig = {
    url: '/system/weberrlog/selectWeberrlog',
    method: 'post',
    data,
    isParams: false,
    bfLoading: false,
...
let deleteByIdReq = (id) => {
  return axiosReq({
    url: '/system/weberrlog/deleteWeberrlog',
    data: { weberrlogId: id },
    isParams: false,
    method: 'post',
    bfLoading: true
...
let tableDelClick = async (row) => {
  await elConfirm('确定', `您确定要删除【${row.pageUrl}】吗？`)
    .then(() => {
      deleteByIdReq(row.weberrlogId).then(() => {
        selectPageReq()
...
const multiDelBtnClick = async () => {
  ...
  axiosReq({
    url: `/system/weberrlog/deleteBatchWeberrlog`,
    data,
    method: 'post',
    bfLoading: true
  }).then(() => {
